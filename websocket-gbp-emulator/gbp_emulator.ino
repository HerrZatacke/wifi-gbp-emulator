...
void gbp_emulator_loop() {
  ...
  if (gbp_printer.packet_ready_flag)
  {
    ...
    {
      case GBP_COMMAND_INIT:
        sprintf(messagePacket, "!{\"command\":\"INIT\"}");
        break;
      case GBP_COMMAND_DATA:
        sprintf(messagePacket, "!{\"command\":\"DATA\",\"compressed\":%d,\"more\":%d}",
            gbp_printer.gbp_packet.compression,
            gbp_printer.gbp_packet.data_length != 0
          );
        break;
      case GBP_COMMAND_PRINT:
        sprintf(messagePacket, "!{\"command\":\"PRNT\",\"sheets\":%d,\"margin_upper\":%d,\"margin_lower\":%d,\"pallet\":%d,\"density\":%d}",
      ...
      case GBP_COMMAND_INQUIRY:
        sprintf(messagePacket, "!{\"command\":\"INQY\",\"status\":{\"lowbatt\":%d,\"jam\":%d,\"err\":%d,\"pkterr\":%d,\"unproc\":%d,\"full\":%d,\"bsy\":%d,\"chk_err\":%d}}",
          gbp_printer.gbp_printer_status.low_battery,
          gbp_printer.gbp_printer_status.paper_jam,
          gbp_printer.gbp_printer_status.other_error,
          gbp_printer.gbp_printer_status.packet_error,
          gbp_printer.gbp_printer_status.unprocessed_data,
      ...
      { // This is called when new data is recieved.
        ...
        */
        ...
        { // Display the data payload encoded in hex
          ...
          char hexVal[2];
          sprintf(hexVal, "%02X", data_8bit);
          imageData = imageData + hexVal;
